...
class BtopCamera(ScryptedDeviceBase, VideoCamera, Settings):
    ...
    async def init_stream(self) -> None:
        ...
        async def run_ffmpeg():
            while True:
                await run_and_stream_output(f'ffmpeg -loglevel error -f x11grab -framerate 15 -draw_mouse 0 -i :{self.virtual_display_num} -c:v libx264 -qp 0 -preset ultrafast -g 60 -an -dn -f flv -listen 1 rtmp://localhost:{self.rtmp_port}/stream', env={'XAUTHORITY': BtopCamera.XAUTH})
